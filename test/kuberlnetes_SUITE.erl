...
post_one_test(_) ->
    R = kuberlnetes:post(
        #{path => "/api/v1/namespaces/default/services", body => #{<<"hello">> => <<"world">>}},
        ?SERVER
    ),
    ?assertMatch({ok, _}, R).
...
post_exists_test(_) ->
    R = kuberlnetes:post(
        #{path => "/api/v1/namespaces/default/services", body => #{<<"already">> => <<"exists">>}},
        ?SERVER
    ),
...
patch_happy_test(_) ->
    R = kuberlnetes:patch(
        #{path => "/api/v1/namespaces/default/services", body => #{<<"happy">> => <<"path">>}},
        ?SERVER
    ),
    ?assertMatch({ok, _}, R).
...
patch_happy_2_test(_) ->
    R = kuberlnetes:patch(
        #{path => "/api/v1/namespaces/default/services", body => #{<<"kdsjfkj">> => <<"sd">>}},
        ?SERVER
    ),
    ?assertMatch({ok, _}, R).
