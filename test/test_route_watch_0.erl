...
handle_req(Req, State, #{
    <<"fieldSelector">> := <<"metadata.name=split">>
}) ->
    Req0 = cowboy_req:stream_reply(
        200,
        #{
    ...
    cowboy_req:stream_body(added_part0(), nofin, Req0),
    timer:sleep(50),
    cowboy_req:stream_body(added_part1(), nofin, Req0),
    timer:sleep(10000),
    {ok, Req0, State};
handle_req(Req, State, #{
...
%% gone test
handle_req(Req, State, #{
    <<"fieldSelector">> := <<"metadata.name=gone">>,
...
added_part0() ->
    <<"{\"object\":{\"apiVersion\":\"v1\",\"data\":{\"key1\":\"new_value1\",\"key2\":\"new_value2\",">>.

added_part1() ->
    <<"\"key3\":\"value3\"},\"kind\":\"ConfigMap\",\"metadata\":{\"name\":\"split\",\"namespace\":\"default\",\"resourceVersion\":\"746850\"}},\"type\":\"ADDED\"}\n">>.
